# -*- coding: utf-8 -*-
"""Eq_of_moti_and_fem.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/124OIoRDfaLkYR1FzjhYUQItjRP60fbGi
"""
...
I1ch = 1/12 * m1ch * (2*l)**2 # то же самое, что и 1/12 * m * (2*l) **2
I2ch = 1/12 * m2ch * (2*l)**2
...
times = np.linspace(0,5,5000) # 4 секунды

vel_prof2 = 0.05 * np.cos(2*np.pi*0.125*times)#0.1/(1+(times-2)**2) * float(1/b_upch)
vel_prof1 = np.zeros_like(vel_prof2)#0.05 * np.cos(2*np.pi*0.125*times+np.pi) #0.1/(1+(times-2)**2) * float(1/b_dwch)
...
t_s1_plt1 = [T_joints_s3[0,i] for i in range(len(T_joints_s1)//2) ]
t_s1_plt2 = [T_joints_s3[1,i] for i in range(len(T_joints_s1)//2) ]
#plt.plot(times,t_s1_#plt1)
#plt.plot(times,t_s1_#plt2)
#plt.show()
