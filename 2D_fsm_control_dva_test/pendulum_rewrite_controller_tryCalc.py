...
import sympy as sym
from sympy import Matrix, init_printing, symbols, solve, eye, Rational, sin, cos, simplify, Eq, evaluate, tanh
...
from formujocoeq_of_moti_and_fem import T_joints_s2
...
while not glfw.window_should_close(window):
    ...
    while (data.time - time_prev < 1.0/60.0):
        ...
        # eq = 3.0*9.81*0.2*np.sin(-Fi_des[1]-Fi_des[0])#(-joint_angular_pos_fi2-joint_angular_pos_fi1) #fi_1 + fi_2
        # eq1 = 3.0*9.81*3/2*0.4*np.sin(-Fi_des[0])#(-joint_angular_pos_fi1) + eq #fi_i (-(np.pi - fi_1))
        ...
        # print("eta_2 ", Ksi_des[1] -  ksi_tau2)
        ...
        if (i<len(T_joints_s2)):
            data.ctrl[0] = T_joints_s2[0,i]#eq1 + float(Tcltr[0])#eq#H eq1 + 
            data.ctrl[1] = T_joints_s2[1,i]#eq + float(Tcltr[1])# eq +
        else:
            data.ctrl[0] = T_joints_s2[0,-1]#eq1 + float(Tcltr[0])#eq#H eq1 + 
            data.ctrl[1] = T_joints_s2[1,-1]#eq + float(Tcltr[1])# eq +
...
# plt.plot(ksides1, label="des 1")
# plt.plot(ArrayKsi_1, label="real 1")
# plt.plot(ksides2, label="desired 2")
# plt.plot(ArrayKsi_2, label="real 2")
...
# plt.tight_layout()
# # plt.show()
# plt.savefig(f"{folder}/two_pendulum2{SH}_{VH}_{taums_1}_{SA}_{VA}_{taums_2}_{A}_{f}.png", bbox_inches='tight', pad_inches=0.5)
# plt.close()
...
# ax.legend()
# fig.tight_layout()
# # fig.savefig(f"{folder}/two_pendulum{SH}_{VH}_{taums_1}_{SA}_{VA}_{taums_2}_{A}_{f}.png", bbox_inches='tight', pad_inches=0.5)
# # plt.close(fig)
# plt.show(fig)
...
# ax.legend()
# fig.tight_layout()
# # fig.savefig(f"{folder}/two_pendulum2{SH}_{VH}_{taums_1}_{SA}_{VA}_{taums_2}_{A}_{f}.png", bbox_inches='tight', pad_inches=0.5)
# # plt.close(fig)
# plt.show(fig)